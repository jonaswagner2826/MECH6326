...
pi_Star = ones(k,k); % Optimal policy
J_Old = ones(k,k); % Optimal costs at next iteration

t = 0; % value iteration counter

%[down, up, right, left]
figure;
while norm(J - J_Old) >= 1e-8 % Value iteration
    t = t + 1 % Count iterations
    J_Old = J; % Update previous cost
    % Policy Evaluation
    A = zeros(k,k);
    b = zeros(k,1);
    x = 0;
    for i = 1:k
        for j = 1:k
            x = x+1; % Count states
            indx = sub2ind(size(J),i,j);
            A(x,indx) = 1;
            b(x) = G(i,j,pi_Star(i,j));
            if pi_Star(i,j) == 1
                if i == 1
                    if j == 1 % Top left corner
                        A(x,sub2ind(size(J),i+1,j)) = -1/2;
                        A(x,sub2ind(size(J),i,j+1)) = -1/2;
                    elseif j == k % Top right corner
                        A(x,sub2ind(size(J),i+1,j)) = -1/2;
                        A(x,sub2ind(size(J),i,j-1)) = -1/2;
                    else % Top boundary
                        A(x,sub2ind(size(J),i+1,j)) = -1/3;
                        A(x,sub2ind(size(J),i,j+1)) = -1/3;
                        A(x,sub2ind(size(J),i,j-1)) = -1/3;
                    end
                elseif i == k
                ...
                elseif j == k % Right boundary
                    ...
                    A(x,sub2ind(size(J),i+1,j)) = -1/4;
                    A(x,sub2ind(size(J),i-1,j)) = -1/4;
                    A(x,sub2ind(size(J),i,j+1)) = -1/4;
                    A(x,sub2ind(size(J),i,j-1)) = -1/4;
                end
            end
        end
...
plot([1 nSims],J_Old(x(1,1),x(2,1))*[1 1])
